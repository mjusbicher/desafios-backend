...
const handleSubmit = (evt, form, route) => {
  ...
  fetch(route, {
    ...
    .then((res) => res.json())
    .then((json) => console.log(json), socket.emit("newProducto", obj));
};
...
Swal.fire({
  title: "Email",
  input: "text",
  text: "Ingresa tu email de identificacion en el chat",
...
socket.on("log", (data) => {
  ...
  Array.from(data).forEach((message) => {
    messages =
      messages +
      `<h6><b>${message.user}</b><h7>  ${message.fecha}: </h7></h6><h8>${message.message}</h8></br>`;
  });
  log.innerHTML = messages;
});
